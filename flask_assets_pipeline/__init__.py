...
class AssetsPipeline:
    ...
    def init_app(
        ...
        state = self.state = AssetsPipelineState(
            ...
            node_modules_path=app.config.get("ASSETS_NODE_MODULES_PATH", node_modules_path),
        ...
        configure_environment(app, inline_assets=state.inline)
    ...
    def bundle(self, assets, name=None, include=False, assets_folder=None, output_folder=None):
        bundles = {}
        includes = None
        ...
        for name, files in bundles.items():
            ...
            for file in files:
                ...
                if is_abs_url(file):
                    ...
                    continue
                if "=" in file:
                    file, outfile = file.split("=", 1)
                path = file
                if assets_folder and not os.path.isabs(file):
                    path = os.path.abspath(os.path.join(assets_folder, file))
                    if not outfile:
                        outfile = file
                if output_folder:
                    outfile = os.path.join(output_folder, outfile or file)
                if outfile:
                    _files.append((path, outfile))
                else:
                    _files.append(path)
            self.state.bundles[name] = _files
            if include:
    ...
    def include(self, path):
        ...
        for p in path:
            if p in self.state.bundles:
                assets.extend(self.bundle_files(p, True))
            else:
                assets.append(p)
